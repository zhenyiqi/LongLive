...
class CausalInferencePipeline(torch.nn.Module):
    ...
    def inference_with_timing(
        ...
        # Start timing if tracker provided
        if latency_tracker:
        ...
        current_start_frame = 0
        self.generator.model.local_attn_size = self.local_attn_size
        print(f"[inference_with_timing] local_attn_size set on model: {self.generator.model.local_attn_size}")
        self._set_all_modules_max_attention_size(self.local_attn_size)
        ...
        # Step 2: Temporal denoising loop with frame timing
        all_num_frames = [self.num_frame_per_block] * num_blocks
        frame_idx = 0
        ...
        for block_idx, current_num_frames in enumerate(all_num_frames):
            ...
            # Record frame completion timing for each frame in the block
            if latency_tracker:
        ...
        if profile:
            ...
            print("Profiling results (with timing):")
            print(f"  - Initialization/caching time: {init_time:.2f} ms ({100 * init_time / total_time:.2f}%)")
            print(f"  - Diffusion generation time: {diffusion_time:.2f} ms ({100 * diffusion_time / total_time:.2f}%)")
            for i, block_time in enumerate(block_times):
                print(f"    - Block {i} generation time: {block_time:.2f} ms ({100 * block_time / diffusion_time:.2f}% of diffusion)")
